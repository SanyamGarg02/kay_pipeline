...
def main():
    ...
    parser.add_argument(
        "--index",
        type=str,
        default="competitor_offers_test",     #chsnge as per your index name or add in command bia cli
    ...
    if args.start_from == "urls":
        cmd = [sys.executable, "kay_jewelers/urls_scraping.py"]
    ...
    if args.start_from in ["urls", "details"]:
        run_step([sys.executable, "kay_jewelers/details_from_urls.py"], "Fetching product details")

    # Step 3: Normalize dataset
    if args.start_from in ["urls", "details", "normalize"]:
        run_step([sys.executable, "kay_jewelers/normalize_dataset.py"], "Normalizing product dataset")

    # Step 4: Prepare embeddings + upload to ES
    if args.start_from in ["urls", "details", "normalize", "embeddings"]:
        run_step(
            [
                sys.executable, "kay_jewelers/prepare_embeddings.py",
