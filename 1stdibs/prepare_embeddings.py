...
# Config
# --------------------------
INPUT_CSV = "1stdibs/poc_1stdibs_normalized.csv"  # 1stdibs normalized CSV
...
NDJSON_FILE = "1stdibs/competitor_offers_1stdibs.ndjson"
...
with open(NDJSON_FILE, "w", encoding="utf-8") as f_ndjson:
    for idx, row in tqdm(df.iterrows(), total=len(df)):
        ...
        action = {
            "_op_type": "update",  # upsert to handle new column
            "_index": ES_INDEX,
            "_id": doc_id,
            "doc": doc,
            "doc_as_upsert": True
...
try:
    ...
    with open("1stdibs/failed_1stdibs_docs.json", "w", encoding="utf-8") as f:
